...
/*   Updated: 2016/03/22 22:41:40 by rmc-coma         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "fractol.h"

static t_fractal	*fo_ship_init(t_fo *obj)
{
	t_fractal	*new;
...
static void			fo_iterate_ship(t_size x, t_size y, t_fo *obj)
{
	t_size		i;
	t_cmpnbr	c;
	t_cmpnbr	z;
	...
	if (i == obj->fractal->max_i)
		...
		obj->frame->buffer[x + y * *(obj->frame->sizeline) / 4] =
			(i * 255 / obj->fractal->max_i) * 0x00010000;
}
